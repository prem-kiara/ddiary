...
export default function Layout({ children, currentPage, onNavigate, pendingCount }) {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <header className="app-header">
        <h1>
          <BookOpen size={24} />
          <span>My Digital Diary</span>
        </h1>
        <div className="header-info">
          {pendingCount > 0 && <span className="badge" style={{ padding: '4px 10px' }}>{pendingCount} pending</span>}
      ...
      <nav className="app-nav">
        {navItems.map(n => (
          ...
          >
            <n.icon size={18} />
            <span>{n.label}</span>
            {n.id === 'tasks' && pendingCount > 0 && <span className="badge">{pendingCount}</span>}
          </button>
        ))}
      </nav>
